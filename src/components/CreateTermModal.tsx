...
interface CreateTermModalProps {
  ...
  onClose: () => void;
  onCreated: (newTerm: TermData) => void;
}
...
export function CreateTermModal({
  ...
  const [termTypeId, setTermTypeId] = useState<number | null>(null);
  ...
  const isValid = termValue.trim() && definition.trim() && example.trim();
      ...
      <ModalHeader bgColor="bg-green-500/10" borderColor="border-green-500/30">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted">Add a new term to the glossary</p>
          </div>
        </div>
      </ModalHeader>
